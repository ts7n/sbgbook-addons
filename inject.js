...
if(document.domain.includes('sbgbook.xyz') && document.URL.includes('student')) {
    const refresh = () => {
        let score = 0;
        let total = 0;
        document.querySelectorAll('.scorehover').forEach((a) => { score += Number(a?.innerHTML.split('<')[0]); total += 10 });
        const gradeText = document.querySelectorAll('.overall-grade-text')[0];
        if(gradeText) {
            gradeText.innerHTML = String(((score / total) * 100).toFixed(2)) + "%";
        }
